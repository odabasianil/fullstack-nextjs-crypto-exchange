...
export const WithdrawCryptoDetail = (props: any) => {
  const { detail, open, setOpen, findStatusText, coins } = props;
  const network = coins?.map((coin: any) => coin?.blockchainList?.find((blockchain: any) => blockchain?.blockchainId === detail?.currencyBlockchainId) ?? false).filter((network: any) => network)
  ...
  return (
    ...
    >
      ...
      <div className="pt-4 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="text-gray-300 dark:text-gray">Network</div>
          <div className="">{network?.[0]?.name}</div>
        </div>
        <div className="flex items-center justify-between text-sm">
          <div className="text-gray-300 dark:text-gray">Address</div>
