...
export const OrderHistory = ({isAccount=false}: {isAccount?: boolean}) => {
  const dates = ['1 Day', '1 Week', '1 Month', '3 Months'];
  ...
  return (
    <div className={twMerge(
      "pt-1 pl-4 md:px-4 h-full",
      isAccount && "md:pl-0"
    )}>
      <div className="lg:min-w-[1284px] overflow-x-auto no-scrollbar">
        {!isAccount && <div className="flex items-center h-10">
          {dates.map((date) => (
            <div
              onClick={() => setSelectedDate(date)}
              className={twMerge(
                "h-6 flex items-center px-2 text-xs text-gray-300 dark:text-gray cursor-pointer rounded-md font-medium whitespace-nowrap",
                selectedDate === date && "text-black dark:text-white bg-white-300 dark:bg-secondary"
              )}
            >
              {date}
            </div>
          ))}
          <div className="pl-2 mr-1 text-xs border-l border-white-300 dark:border-secondary text-gray-300 dark:text-gray underline decoration-dashed relative group cursor-help">
            Time
            <HelpText isTop={false}>
              Displays history for the past 6 months only. Use the "Export " function in orders page to export more data.
            </HelpText>
          </div>
          ...
          <div className="ml-3 flex items-center">
            ...
            <button className="text-xs rounded-md ml-0.5 h-6 flex items-center">Reset</button>
          </div>
        </div>}
        <div className="pt-1 h-8 flex items-center justify-start">
          ...
          <div className="flex items-center justify-start flex-grow w-[60px] min-w-[60px]">
            <div className="text-black-300 dark:text-gray text-xs">Average</div>
          </div>
        ...
        <div className="w-full">
          {new Array(10).fill(null).map((idx) => (
            <div className={twMerge(
              ...
              <div className="flex items-center justify-start flex-grow w-[100px] min-w-[100px] px-2 max-w-[100px] z-20 text-xs">
                Cancelled
              </div>
            </div>
          ))}
